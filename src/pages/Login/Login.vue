...
export default {
  data() {
    return {
      loginWay: false,  //默认为短信登陆
      computeTime: 0, //倒计时计数，当前没有计时默认为0
      showPassword: false,
      phone: '', //手机号
      pwd: '',//密码
      name: '', //用户名
  ...
  methods: {
    ...
    async login() {
      ...
      if(result.code === 0) {
        ...
        const msg = result.msg
        this.showAlert(msg)
        this.getCaptcha() //显示新的验证图片
      }
    },
    getCaptcha(){
      //每次指定的src不一样
      return this.$refs.captcha.target.src = "http://localhost:4000/captcha?time=" + new Date()
    }
  },
